...
<div id="content">
  ...
  <p class="form-error-message mt-4" v-show="formError">{{errorMessage}}</p>
</div>
</template>
  ...
  export default {
    ...
    metaInfo () {
      return {
        ...
        meta: [ {
          name:"description",
          content:"Download our media pack and see how your business can benefit from roundabout sponsorship, bus shelter advertising, car parking ticket adverts and more."
        } ]
      }
    },
    methods: {
        ...
        if (!this.formHasErrors) {
          ...
          this.$recaptcha('login').then((token) => {
            emailService.sending(text,token).then((resp) => {
              ...
              this.errorMessage = "An error has occured, try again later, or you can get in touch by phone: 01305 224125 or email: marketing@dorsetcouncil.gov.uk";
              this.formError = true;
            })
          }).catch((err)=>{
